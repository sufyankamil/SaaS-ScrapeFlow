...
function FlowEditor({ workflow }: { workflow: Workflow }) {
    const [nodes, setNodes, onNodesChange] = useNodesState<AppNode>([CreateFlowNode(TaskType.LAUNCH_BROWSER)]);
    ...
    useEffect(() => {
        try {
            ...
            const flow = savedFlow ? JSON.parse(savedFlow) : JSON.parse(workflow.definition);
            if (!flow) return;
    ...
    useEffect(() => {
        const updatedWorkflow = JSON.stringify({ nodes, edges, viewport: { x: 0, y: 0, zoom: 1 } });
    ...
    const isValidConnection = useCallback((connection: Edge | Connection) => {
        ...
        if (!source || !target) {
            console.log("Invalid connetion: Source or target not found");
            return false;
        }
        ...
        const output = targetTask.inputs.find((o) => o.name === connection.sourceHandle);
